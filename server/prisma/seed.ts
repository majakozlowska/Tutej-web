...
async function main() {
    ...
    await Promise.all([
        prisma.comment.create({ data: { content: 'Ja chętnie!', authorId: users[0].id, postId: posts[0].id } }),
        prisma.comment.create({ data: { content: 'Widziałem ogłoszenie na klatce.', authorId: users[1].id, postId: posts[1].id } }),
        prisma.comment.create({ data: { content: 'Dzięki za polecenie!', authorId: users[2].id, postId: posts[2].id } }),
        prisma.comment.create({ data: { content: 'Wreszcie to naprawiają.', authorId: users[3].id, postId: posts[3].id } }),
        prisma.comment.create({ data: { content: 'Potwierdzam, są genialne.', authorId: users[0].id, postId: posts[4].id } }),
    ]);
